...
public class ZoomableDraweeView extends DraweeView<GenericDraweeHierarchy>
    ...
    private static final long DOUBLE_CLICK_HALF_DELAY = 400;

    private final RectF mImageBounds = new RectF();
    private final RectF mViewBounds = new RectF();
    private boolean fastFirstClick = false;
    private boolean fastSecondClick = false;
    ...
    private float initialScaleFactor = Float.MIN_VALUE;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                if (fastSecondClick) {
                    if (initialScaleFactor == Float.MIN_VALUE) {
                        zoomIn();
                    } else {
                        zoomOut();
                    }
    ...
    private void secondClickActivate() {
        if (!fastSecondClick) {
            fastSecondClick = true;
            Observable.timer(DOUBLE_CLICK_HALF_DELAY, TimeUnit.MILLISECONDS).subscribe(new Action1<Long>() {
                @Override
                public void call(Long aLong) {
                    fastSecondClick = false;
                }
            });
        }
    }

    private void firstClickActivate() {
        if (!fastFirstClick) {
            fastFirstClick = true;
            Observable.timer(DOUBLE_CLICK_HALF_DELAY, TimeUnit.MILLISECONDS).subscribe(new Action1<Long>() {
                @Override
                public void call(Long aLong) {
                    fastFirstClick = false;
                }
            });
        }
    }
    ...
    private void zoomIn() {
        initialScaleFactor = mZoomableController.getScaleFactor();
        ((DefaultZoomableController) mZoomableController).zoomToImagePoint(
                4f, new PointF(0.5f, 0.5f), new PointF(getMeasuredWidth() / 2, getMeasuredHeight() / 2), false, false, 0, null
        );
    }

    private void zoomOut() {
        ((DefaultZoomableController) mZoomableController).zoomToImagePoint(
                initialScaleFactor, new PointF(0.5f, 0.5f), new PointF(getMeasuredWidth() / 2, getMeasuredHeight() / 2), false, false, 0, null
        );
        initialScaleFactor = Float.MIN_VALUE;
    }
}